...
<section class="w-2/3 md:w-2/3 flex flex-col items-center px-3 mx-auto">
    <article class="flex flex-col shadow my-4">
        <div class="bg-white flex flex-col justify-start p-6">
            <span class="text-3xl font-bold hover:text-gray-700 pb-4">{{$details->title}}</span>
            <p class="text-sm pb-8">
                By <a class="font-semibold hover:text-gray-800">{{$details->user->name}}</a>, Published on
                {{$details->publication_date->format('M d, Y')}}
            </p>
            {!!$details->description!!}
        </div>
    </article>
    ...
    <div class="w-full flex flex-col text-center md:text-left md:flex-row shadow bg-white mt-10 mb-10 p-6">
        <div class="flex-1 flex flex-col justify-center md:justify-start">
            <p class="font-semibold text-2xl">{{$details->user->name}}</p>
